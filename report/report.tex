...
\begin{lstlisting}[firstnumber=7]  
  while (frontier.Count > 0) {
    ...
    foreach (var action in actions) { //Knuden udforskes *** også frontier tjek?
...
Kørsel af en rutefinding foregår ved først at parse en tekstfil til en knowledge base. Den KB, sammen med et \code{StartState} og \code{EndState} gives som parametre til \code{AStar.Search()}, som returnerer et \code{SearchResult}. \code{SearchResult} indeholder den funde mål-\code{Node}, hvis der findes en sti dertil.
...
foreach (var firstLiteral in parent.State.Clause) {
  foreach (var secondLiteral in action.Clause) {
    if (firstLiteral.Name.Equals(secondLiteral.Name)
      ...
      // Fjerner duplikater, f.eks. $A \lor A \lor C$ -> $A \lor C$
...
\subsection{Output}
Outputtet er ikke af nogen særlig interesse, med undtagelse af hvor mange iterationer der blev brugt til at finde løsningen, hvilket er det sidste tal i hver test linie. Følgende er outputtet af alle tests
\begin{verbatim}
--- Inference ---
Test: Simple success: SUCCESS (solved 1)
Test: Simple failure: SUCCESS (not solved 0)
Test: Breakfast: SUCCESS (solved 15)
Test: Ancestor (pq): SUCCESS (solved 6)
Test: Espresso light: SUCCESS (solved 9)
Test: Steam: SUCCESS (solved 29)
Test: No steam (boiler off): SUCCESS (solved 6)
Test: No steam (boiler broken): SUCCESS (solved 5)
Test: Espresso: SUCCESS (solved 40)
Passed 9 of 9 (100%)

--- Route finding ---
Test: Manhattan ... Parsing ... Solving: SUCCESS (solved 66)
Test: Copenhagen ... Parsing ... Solving: SUCCESS (solved 13)
Test: Copenhagen ... Parsing ... Solving: SUCCESS (solved 7)
Test: Romania ... Parsing ... Solving: SUCCESS (solved 4)
Test: Romania ... Parsing ... Solving: SUCCESS (solved 4)
Test: Biggie ... Parsing ... Solving: SUCCESS (solved 9999)
Passed 6 of 6 (100%)
\end{verbatim}
